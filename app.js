...
async function CashbackWallet(nationalID){
  const pool = await sql.connect(config)
  const result = await pool.request().input('NID', sql.Int, nationalID).query('SELECT * FROM dbo.Cashback_Wallet_Customer(@NID)')
...
async function notResolvedTickets(mobileNumber) {
  try {
    const pool = await sql.connect(config);
    const nationalIDres = await pool.request().query(`SELECT nationalID FROM customer_account WHERE mobileNo = ${mobileNumber}`);
...
async function remAmount(paymentID, planID) {
  try {
    ...
    .query('SELECT dbo.function_remaining_amount(@paymentId, @planId) AS res');
...
async function averageSentTransactions(startDate, endDate, walletID){
  ...
  const result = await pool.request()
                        ...
                        .input('walletID', sql.Int, walletID)
                        .query('SELECT dbo.Wallet_Transfer_Amount(@walletID, @start_date, @end_date) AS average');
